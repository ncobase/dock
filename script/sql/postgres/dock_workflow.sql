...
(
    ...
    is_publish      int2         NOT NULL DEFAULT 0,                       -- 是否发布（0 未发布 1 已发布 9 失效）
    form_custom     bpchar(1)    NULL     DEFAULT 'N':: character varying, -- 审批表单是否自定义（Y 是 N 否）
    form_path       varchar(100) NULL,                                     -- 审批表单路径
    activity_status int2         NOT NULL DEFAULT 1,                       -- 流程激活状态（0 挂起 1 激活）
    ...
    tenant_id       varchar(40)  NULL,                                     -- 租户 id
...
COMMENT
    ON COLUMN flow_definition.id IS '主键 id';
COMMENT
...
COMMENT
    ON COLUMN flow_definition.is_publish IS '是否发布（0 未发布 1 已发布 9 失效）';
COMMENT
    ON COLUMN flow_definition.form_custom IS '审批表单是否自定义（Y 是 N 否）';
COMMENT
    ON COLUMN flow_definition.form_path IS '审批表单路径';
COMMENT
    ON COLUMN flow_definition.activity_status IS '流程激活状态（0 挂起 1 激活）';
COMMENT
...
COMMENT
    ON COLUMN flow_definition.tenant_id IS '租户 id';

CREATE TABLE flow_node
(
    id              int8          NOT NULL,                             -- 主键 id
    node_type       int2          NOT NULL,                             -- 节点类型（0 开始节点 1 中间节点 2 结束节点 3 互斥网关 4 并行网关）
    definition_id   int8          NOT NULL,                             -- 流程定义 id
    ...
    skip_any_node   varchar(100)  NULL DEFAULT 'N':: character varying, -- 是否可以退回任意节点（Y 是 N 否）即将删除
...
COMMENT
    ON COLUMN flow_node.id IS '主键 id';
COMMENT
    ON COLUMN flow_node.node_type IS '节点类型（0 开始节点 1 中间节点 2 结束节点 3 互斥网关 4 并行网关）';
COMMENT
    ON COLUMN flow_node.definition_id IS '流程定义 id';
COMMENT
    ON COLUMN flow_node.node_code IS '流程节点编码';
COMMENT
    ON COLUMN flow_node.node_name IS '流程节点名称';
COMMENT
    ON COLUMN flow_node.permission_flag IS '权限标识（权限类型：权限标识，可以多个，用逗号隔开)';
COMMENT
    ON COLUMN flow_node.node_ratio IS '流程签署比例值';
COMMENT
    ON COLUMN flow_node.coordinate IS '坐标';
COMMENT
    ON COLUMN flow_node.skip_any_node IS '是否可以退回任意节点（Y 是 N 否）即将删除';
COMMENT
...
COMMENT
    ON COLUMN flow_node.form_custom IS '审批表单是否自定义（Y 是 N 否）';
COMMENT
...
COMMENT
    ON COLUMN flow_node.tenant_id IS '租户 id';
...
(
    ...
    now_node_type  int2         NULL,                                 -- 当前节点类型（0 开始节点 1 中间节点 2 结束节点 3 互斥网关 4 并行网关）
    next_node_code varchar(100) NOT NULL,                             -- 下一个流程节点的编码
    next_node_type int2         NULL,                                 -- 下一个节点类型（0 开始节点 1 中间节点 2 结束节点 3 互斥网关 4 并行网关）
    skip_name      varchar(100) NULL,                                 -- 跳转名称
    skip_type      varchar(40)  NULL,                                 -- 跳转类型（PASS 审批通过 REJECT 退回）
...
COMMENT
    ON COLUMN flow_skip.id IS '主键 id';
COMMENT
    ON COLUMN flow_skip.definition_id IS '流程定义 id';
COMMENT
    ON COLUMN flow_skip.now_node_code IS '当前流程节点的编码';
COMMENT
    ON COLUMN flow_skip.now_node_type IS '当前节点类型（0 开始节点 1 中间节点 2 结束节点 3 互斥网关 4 并行网关）';
COMMENT
    ON COLUMN flow_skip.next_node_code IS '下一个流程节点的编码';
COMMENT
    ON COLUMN flow_skip.next_node_type IS '下一个节点类型（0 开始节点 1 中间节点 2 结束节点 3 互斥网关 4 并行网关）';
COMMENT
    ON COLUMN flow_skip.skip_name IS '跳转名称';
COMMENT
    ON COLUMN flow_skip.skip_type IS '跳转类型（PASS 审批通过 REJECT 退回）';
COMMENT
...
COMMENT
    ON COLUMN flow_skip.tenant_id IS '租户 id';

CREATE TABLE flow_instance
(
    id              int8         NOT NULL,                                 -- 主键 id
    definition_id   int8         NOT NULL,                                 -- 对应 flow_definition 表的 id
    business_id     varchar(40)  NOT NULL,                                 -- 业务 id
    node_type       int2         NOT NULL,                                 -- 节点类型（0 开始节点 1 中间节点 2 结束节点 3 互斥网关 4 并行网关）
    node_code       varchar(40)  NOT NULL,                                 -- 流程节点编码
    node_name       varchar(100) NULL,                                     -- 流程节点名称
    variable        text         NULL,                                     -- 任务变量
    flow_status     varchar(20)  NOT NULL,                                 -- 流程状态（0 待提交 1 审批中 2 审批通过 8 已完成 9 已退回 10 失效）
    activity_status int2         NOT NULL DEFAULT 1,                       -- 流程激活状态（0 挂起 1 激活）
    def_json        text         NULL,                                     -- 流程定义 json
...
COMMENT
    ON COLUMN flow_instance.id IS '主键 id';
COMMENT
    ON COLUMN flow_instance.definition_id IS '对应 flow_definition 表的 id';
COMMENT
    ON COLUMN flow_instance.business_id IS '业务 id';
COMMENT
    ON COLUMN flow_instance.node_type IS '节点类型（0 开始节点 1 中间节点 2 结束节点 3 互斥网关 4 并行网关）';
COMMENT
...
COMMENT
    ON COLUMN flow_instance.flow_status IS '流程状态（0 待提交 1 审批中 2 审批通过 3 自动通过 4 终止 5 作废 6 撤销 7 取回  8 已完成 9 已退回 10 失效）';
COMMENT
    ON COLUMN flow_instance.activity_status IS '流程激活状态（0 挂起 1 激活）';
COMMENT
    ON COLUMN flow_instance.def_json IS '流程定义 json';
COMMENT
...
COMMENT
    ON COLUMN flow_instance.tenant_id IS '租户 id';

CREATE TABLE flow_task
(
    id            int8         NOT NULL,                             -- 主键 id
    definition_id int8         NOT NULL,                             -- 对应 flow_definition 表的 id
    instance_id   int8         NOT NULL,                             -- 对应 flow_instance 表的 id
...
COMMENT
    ON COLUMN flow_task.id IS '主键 id';
COMMENT
    ON COLUMN flow_task.definition_id IS '对应 flow_definition 表的 id';
COMMENT
    ON COLUMN flow_task.instance_id IS '对应 flow_instance 表的 id';
COMMENT
    ON COLUMN flow_task.node_code IS '节点编码';
COMMENT
    ON COLUMN flow_task.node_name IS '节点名称';
COMMENT
    ON COLUMN flow_task.node_type IS '节点类型（0 开始节点 1 中间节点 2 结束节点 3 互斥网关 4 并行网关）';
COMMENT
    ON COLUMN flow_task.form_custom IS '审批表单是否自定义（Y 是 N 否）';
COMMENT
...
COMMENT
    ON COLUMN flow_task.tenant_id IS '租户 id';
...
(
    ...
    definition_id    int8         NOT NULL,                                 -- 对应 flow_definition 表的 id
    instance_id      int8         NOT NULL,                                 -- 对应 flow_instance 表的 id
    task_id          int8         NOT NULL,                                 -- 对应 flow_task 表的 id
    node_code        varchar(200) NULL,                                     -- 开始节点编码
    node_name        varchar(200) NULL,                                     -- 开始节点名称
    node_type        int2         NULL,                                     -- 开始节点类型（0 开始节点 1 中间节点 2 结束节点 3 互斥网关 4 并行网关）
    target_node_code varchar(200) NULL,                                     -- 目标节点编码
    target_node_name varchar(200) NULL,                                     -- 结束节点名称
    approver         varchar(40)  NULL,                                     -- 审批者
    cooperate_type   int2         NOT NULL DEFAULT 0,                       -- 协作方式 (1 审批 2 转办 3 委派 4 会签 5 票签 6 加签 7 减签)
    collaborator     varchar(40)  NULL,                                     -- 协作人 (只有转办、会签、票签、委派)
    skip_type        varchar(10)  NULL,                                     -- 流转类型（PASS 通过 REJECT 退回 NONE 无动作）
    flow_status      varchar(20)  NOT NULL,                                 -- 流程状态（0 待提交 1 审批中 2 审批通过 8 已完成 9 已退回 10 失效）
    form_custom      bpchar(1)    NULL     DEFAULT 'N':: character varying, -- 审批表单是否自定义（Y 是 N 否）
    form_path        varchar(100) NULL,                                     -- 审批表单路径
    ext              varchar(500) NULL,                                     -- 扩展字段，预留给业务系统使用
...
COMMENT
    ON COLUMN flow_his_task.id IS '主键 id';
COMMENT
    ON COLUMN flow_his_task.definition_id IS '对应 flow_definition 表的 id';
COMMENT
    ON COLUMN flow_his_task.instance_id IS '对应 flow_instance 表的 id';
COMMENT
    ON COLUMN flow_his_task.task_id IS '对应 flow_task 表的 id';
COMMENT
    ON COLUMN flow_his_task.node_code IS '开始节点编码';
COMMENT
    ON COLUMN flow_his_task.node_name IS '开始节点名称';
COMMENT
    ON COLUMN flow_his_task.node_type IS '开始节点类型（0 开始节点 1 中间节点 2 结束节点 3 互斥网关 4 并行网关）';
COMMENT
...
COMMENT
    ON COLUMN flow_his_task.cooperate_type IS '协作方式 (1 审批 2 转办 3 委派 4 会签 5 票签 6 加签 7 减签)';
COMMENT
    ON COLUMN flow_his_task.collaborator IS '协作人';
COMMENT
    ON COLUMN flow_his_task.skip_type IS '流转类型（PASS 通过 REJECT 退回 NONE 无动作）';
COMMENT
    ON COLUMN flow_his_task.flow_status IS '流程状态（1 审批中 2 审批通过 9 已退回 10 失效）';
COMMENT
    ON COLUMN flow_his_task.form_custom IS '审批表单是否自定义（Y 是 N 否）';
COMMENT
...
COMMENT
    ON COLUMN flow_his_task.tenant_id IS '租户 id';

CREATE TABLE flow_user
(
    id           int8        NOT NULL,                             -- 主键 id
    "type"       bpchar(1)   NOT NULL,                             -- 人员类型（1 待办任务的审批人权限 2 待办任务的转办人权限 3 流程实例的抄送人权限 4 待办任务的委托人权限）
    processed_by varchar(80) NULL,                                 -- 权限人
    associated   int8        NOT NULL,                             -- 任务表 id
    create_time  timestamp   NULL,                                 -- 创建时间
...
COMMENT
    ON COLUMN flow_user.id IS '主键 id';
COMMENT
    ON COLUMN flow_user."type" IS '人员类型（1 待办任务的审批人权限 2 待办任务的转办人权限 3 待办任务的委托人权限）';
COMMENT
    ON COLUMN flow_user.processed_by IS '权限人';
COMMENT
    ON COLUMN flow_user.associated IS '任务表 id';
COMMENT
...
COMMENT
    ON COLUMN flow_user.tenant_id IS '租户 id';
...
COMMENT
    ON COLUMN flow_category.category_id IS '流程分类 ID';
COMMENT
    ON COLUMN flow_category.tenant_id IS '租户编号';
COMMENT
    ON COLUMN flow_category.parent_id IS '父流程分类 id';
COMMENT
...
COMMENT
    ON COLUMN flow_category.del_flag IS '删除标志（0 代表存在 1 代表删除）';
COMMENT
...
VALUES (100, '000000', 0, '0', 'OA 审批', 0, '0', 103, 1, now(), NULL, NULL);
INSERT INTO flow_category
