...
import com.ncobase.framework.core.domain.dto.RoleDTO;
...
import com.ncobase.framework.mybatis.enums.DataScopeType;
import com.ncobase.framework.mybatis.helper.DataPermissionHelper;
...
public class PlusDataPermissionHandler {
    ...
    /**
     * 类名称与注解的映射关系缓存 (由于 aop 无法拦截 mybatis 接口类上的注解 只能通过启动预扫描的方式进行)
     */
    ...
    /**
     * bean 解析器 用于处理 spel 表达式中对 bean 的调用
     */
    ...
    private String buildDataFilter(DataPermission dataPermission, boolean isSelect) {
        ...
        for (DataColumn dataColumn : dataPermission.value()) {
            if (dataColumn.key().length != dataColumn.value().length) {
                throw new ServiceException("角色数据范围异常 => key 与 value 长度不匹配");
            }
            // 包含权限标识符 这直接跳过
        ...
        for (RoleDTO role : user.getRoles()) {
            ...
            for (DataColumn dataColumn : dataPermission.value()) {
                ...
                if (ignoreMap.containsKey(dataColumn)) {
                    // 修复多角色与权限标识符共用问题 https://github.com/ncobase/dock/issues/IB4CS4
                ...
                // 忽略数据权限 防止 spel 表达式内有其他 sql 查询导致死循环调用
    ...
    /**
     * 对所有 null 变量找不到的变量返回默认值 委托模式 将不需要处理的方法委托给原处理器
     */
    @AllArgsConstructor
