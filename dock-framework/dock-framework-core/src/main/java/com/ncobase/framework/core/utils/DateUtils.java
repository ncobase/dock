...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
    ...
    /**
     * 获取当前日期和时间的字符串表示，格式为 YYYYMMDDHHMMSS
     *
     ...
     * @return 转换后的日期对象，如果转换失败或输入为 null，则返回 null
     */
    ...
    /**
     * 计算两个时间点的差值（天、小时、分钟、秒），当值为 0 时不显示该单位
     *
     * @param endDate 结束时间
     * @param nowDate 当前时间
     * @return 时间差字符串，格式为 "x 天 x 小时 x 分钟 x 秒"，若为 0 则不显示
     */
    public static String getTimeDifference(Date endDate, Date nowDate) {
        ...
        return result.length() > 0 ? result.toString().trim() : "0 秒";
    }
