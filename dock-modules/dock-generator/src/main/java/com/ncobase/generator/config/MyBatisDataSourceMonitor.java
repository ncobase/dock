package com.ncobase.generator.config;
...
public class MyBatisDataSourceMonitor implements DataSourceMonitor {
    ...
    /**
     * 数据源特征 用来定准 adapter 包含数据库或 JDBC 协议关键字<br/>
     * 一般会通过 产品名_url 合成 如果返回 null 上层方法会通过 driver_产品名_url 合成
     *
     * @param datasource 数据源
     * @return String 返回 null 由上层自动提取
     */
    @Override
    ...
    /**
     * ConfigTable.KEEP_ADAPTER=2 : 根据当前接口判断是否保持同一个数据源绑定同一个 adapter<br/>
     * DynamicRoutingDataSource 类型的返回 false，因为同一个 DynamicRoutingDataSource 可能对应多类数据库，如果项目中只有一种数据库 应该直接返回 true
     *
     * @param datasource 数据源
     * @return boolean
     */
